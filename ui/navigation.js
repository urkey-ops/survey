...
// Converted to browser-global (no ES module exports)
...
(function () {
  ...
  const navigationModule = {
    ...
    cleanupInputFocusScroll,
    cleanupIntervals,
    // expose internal helpers for tests / other modules if needed
    _internal: {
      saveState,
      updateData,
      startQuestionTimer,
  ...
  window.uiHandlers = window.uiHandlers || {};

  // Attach functions directly to uiHandlers so existing main.js destructuring works
  Object.keys(navigationModule).forEach((key) => {
    // avoid overwriting if another module intentionally set an implementation already
    if (window.uiHandlers[key] === undefined) {
      window.uiHandlers[key] = navigationModule[key];
    } else {
      // keep existing if present; but still make available under navigationHandler
      console.warn(`[NAVIGATION] uiHandlers.${key} already exists — keeping existing implementation.`);
    }
  });
})();
